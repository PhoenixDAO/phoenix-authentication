...
contract('ClientRaindrop', function (accounts) {
  ...
  const permissionString = 'Create RaindropClient Phoenix Account'
  ...
  it('raindrop client deployed and linked to the Phoenix token', async function () {
    raindropInstance = await ClientRaindrop.new({from: owner.public})
  })

  it('raindrop linked to token', async function () {
    await raindropInstance.setPhoenixTokenAddress(phoenixInstance.address, {from: owner.public})
    let contractPhoenixTokenAddress = await raindropInstance.phoenixTokenAddress()
  ...
  it('staking minimums are settable', async function () {
    await raindropInstance.setMinimumPhoenixStakes(
      minimumPhoenixStakeUser, minimumPhoenixStakeDelegatedUser, {from: owner.public}
    )
    let contractMinimumPhoenixStakeUser = await raindropInstance.minimumPhoenixStakeUser()
    let contractMinimumPhoenixStakeDelegatedUser = await raindropInstance.minimumPhoenixStakeDelegatedUser()
